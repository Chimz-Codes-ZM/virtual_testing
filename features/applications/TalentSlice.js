...
const TalentSlice = createSlice({
  ...
  reducers: {
    ...
    toggleApproved: (state, action) => {
      ...
      state.approvedApplicants[applicantId] =
        !state.approvedApplicants[applicantId];
    ...
    toggleDenied: (state, action) => {
      const { applicantId, accountType } = action.payload;
      ...
      state.deniedApplicants[applicantId] =
        !state.deniedApplicants[applicantId];
    ...
    setAccountType: (state, action) => {
      ...
      // Check if the user is in approvedApplicants
      if (state.approvedApplicants[applicantId]) {
          state.approvedApplicants[applicantId] = {
              ...state.approvedApplicants[applicantId],
              account_type: accountType,
          };
      } else if (state.deniedApplicants[applicantId]) {
          state.deniedApplicants[applicantId] = {
              ...state.deniedApplicants[applicantId],
              account_type: accountType,
          };
