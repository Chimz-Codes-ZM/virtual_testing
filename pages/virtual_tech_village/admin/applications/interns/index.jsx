...
import {
  ...
  toggleDenied,
  resetState,
} from "../../../../../features/applications/InternSlice";
import Skeleton from "../../components/skeleton";
import { API_URL } from "@/config";

const Index = () => {
  const dispatch = useDispatch();
  const state = useSelector((state) => state.internApplications);
  ...
  async function fetchData() {
    try {
      const response = await axios.get(
        `https://${API_URL}/village/intern_approval/${user.user_id}/`
      );
      ...
      <Layout sideHighlight="Insight">
        ...
        <div className="w-full custom-height flex flex-col gap-5 mt-16 overflow-x-hidden">
          ...
          <div className="w-full flex flex-col relative">
            ...
            >
              {info?.approved_interns?.map((applicant) => (
                <div
                  key={applicant.id}
            ...
            >
              {info?.denied_interns?.map((applicant) => (
                <div
                  key={applicant.id}
                  className="w-full flex border-b py-2 border-gray-600"
                >
